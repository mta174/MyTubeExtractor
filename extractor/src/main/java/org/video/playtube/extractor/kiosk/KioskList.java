...
public  class KioskList {
    private static final String TAG = KioskList.class.getSimpleName();
    public interface KioskExtractorFactory {
        KioskExtractor createNewKiosk(final StreamingService streamingService, final String url, final String kioskId, final Localization localization) throws ExtractionException, IOException;
    }
    ...
    public void addKioskEntry(KioskExtractorFactory extractorFactory, ListLinkHandlerFactory handlerFactory, String id) throws Exception {
    ...
    public KioskExtractor getDefaultKioskExtractor(String nextPageUrl) throws ExtractionException, IOException {
        return getDefaultKioskExtractor(nextPageUrl, PlayTube.getPreferredLocalization());
    }

    public KioskExtractor getDefaultKioskExtractor(String nextPageUrl, Localization localization) throws ExtractionException, IOException {
    ...
    public KioskExtractor getExtractorById(String kioskId, String nextPageUrl, Localization localization) throws ExtractionException, IOException {
        //LogHelper.i(TAG, "getExtractorById", "kioskId", kioskId, "nextPageUrl", nextPageUrl);
        KioskEntry ke = kioskList.get(kioskId);
        if(ke == null) {
            throw new ExtractionException("No kiosk found with the type: " + kioskId);
        } else {
            return ke.extractorFactory.createNewKiosk(PlayTube.getService(service_id), ke.handlerFactory.fromId(kioskId).getUrl(), kioskId, localization);
        }
    }
    ...
    public KioskExtractor getExtractorByUrl(String url, String nextPageUrl) throws ExtractionException, IOException{
        return getExtractorByUrl(url, nextPageUrl, PlayTube.getPreferredLocalization());
    }

    public KioskExtractor getExtractorByUrl(String url, String nextPageUrl, Localization localization) throws ExtractionException, IOException {
