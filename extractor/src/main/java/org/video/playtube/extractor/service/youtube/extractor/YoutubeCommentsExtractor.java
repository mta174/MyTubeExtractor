...
import org.video.playtube.extractor.DownloadRequest;
import org.video.playtube.extractor.DownloadResponse;
...
import org.video.playtube.extractor.comment.CommentsInfoItem;
import org.video.playtube.extractor.comment.CommentsInfoItemExtractor;
import org.video.playtube.extractor.comment.CommentsInfoItemsCollector;
...
import org.video.playtube.extractor.util.JsonUtils;
...
public class YoutubeCommentsExtractor extends CommentsExtractor {
    ...
    // using the mobile site for comment because it loads faster and uses get requests instead of post
    ...
    public InfoItemsPage<CommentsInfoItem> getInitialPage() throws IOException, ExtractionException {
        // initial page does not load any comment but is required to get comment token
        super.fetchPage();
        return initPage;
    }
    ...
    public InfoItemsPage<CommentsInfoItem> getPage(String pageUrl) throws IOException, ExtractionException {
        ...
        try {
            ...
            throw new ParsingException("Could not parse json data for comment", e);
        }
    ...
    private String makeAjaxRequest(String siteUrl) throws IOException, ReCaptchaException {
        ...
        return PlayTube.getDownloader().get(siteUrl, request).getResponseBody();
    }
    ...
    private String getDataString(Map<String, String> params) throws UnsupportedEncodingException {
        ...
        for (Map.Entry<String, String> entry : params.entrySet()) {
            if (first)
                ...
                result.append("&");
            result.append(URLEncoder.encode(entry.getKey(), ExtractorConstant.CHARSET_UTF_8));
            result.append("=");
            result.append(URLEncoder.encode(entry.getValue(), ExtractorConstant.CHARSET_UTF_8));
        }
