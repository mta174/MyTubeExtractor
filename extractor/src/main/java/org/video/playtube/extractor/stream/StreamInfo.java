...
import org.video.playtube.extractor.util.DashMpdParser;
...
public class StreamInfo extends Info {
    ...
    private static StreamInfo extractImportantData(StreamExtractor extractor) throws ExtractionException {
        ...
        if ((streamType == StreamType.NONE) || (url == null || url.isEmpty()) || (id == null || id.isEmpty())
            ...
            throw new ExtractionException("Some important streams information was not given.");
        }
    ...
    private static StreamInfo extractStreams(StreamInfo streamInfo, StreamExtractor extractor)
        ...
        /* ---- streams extraction goes here ---- */
        // At least one type of streams has to be available,
        // otherwise an exception will be thrown directly into the frontend.
        streamInfo.setUseYoutubeApi(extractor.isUserYoutubeApi());
        try {
        ...
        /* Extract video only streams url */
        try {
        ...
        if(!streamInfo.isUseYoutubeApi()) {
            ...
            // Either audio or video has to be available, otherwise we didn't get a streams
            ...
            if ((streamInfo.videoStreams.isEmpty()) && (streamInfo.audioStreams.isEmpty())) {
                ...
                throw new StreamExtractException("Could not get any streams. See error variable to get further details.");
            }
        }
    ...
    private boolean isUseYoutubeApi = false;
    ...
    public boolean isUseYoutubeApi() {
        return isUseYoutubeApi;
    }

    public void setUseYoutubeApi(boolean useYoutubeApi) {
        isUseYoutubeApi = useYoutubeApi;
    }
